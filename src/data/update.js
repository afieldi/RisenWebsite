        ...
        fetch(champ_url).then(response2 => {
            response2.json().then(champData => {
                ...
                for (let key in champData["data"]) {
                    ...
                    if(!fs.existsSync(`src/images/champions/icons/${key}_0.jpg`)) {
                        fetch(`http://ddragon.leagueoflegends.com/cdn/${version}/img/champion/${key}.jpg`).then(res => {
                            const dest = fs.createWriteStream(`src/images/champions/icons/${key}_0.jpg`);
                            res.body.pipe(dest);
                        });
                    }
